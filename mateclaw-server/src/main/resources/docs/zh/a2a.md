...
AuraClaw 可以把已启用智能体暴露为 A2A JSON-RPC 端点，也可以通过工具调用其他 A2A 对端。
...
生产环境必须配置 `base-url`。为空时，AuraClaw 会按入站请求推导名片 URL，这依赖反向代理正确传递 Host 与协议头。
...
- `POST /api/a2a` 复用 AuraClaw Bearer token 鉴权，支持 `message/send`、`message/stream`、`tasks/get`、`tasks/cancel`。
