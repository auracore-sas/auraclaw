...
聊天是你真正干活的地方。AuraClaw 里其他所有东西——Agent、工具、记忆、Wiki、渠道——存在的理由，都是为了让**这个框里发生的事**足够好。
...
AuraClaw 的聊天 UI 在试着回答一个问题：**AI 刚刚告诉你的事情，该不该信？** 别处的默认答案是"看答案，自己猜"。AuraClaw 想做得更好。
...
在桌面端**拖一个文件夹**进来，Agent 拿到的是这个文件夹的绝对路径引用——它可以用文件读取或 shell 工具去遍历里面的文件。在 Web 端拖文件夹进来，AuraClaw 会递归展开、把每个文件单独上传。
...
一个会话是属于某个 Agent + 某个用户的消息序列。AuraClaw 用两张表存它们：
...
每一个回合，AuraClaw 都会构造真正送进 LLM 的那个 prompt。大致是：
