...
description: AuraClaw Team Run 用一个 runId 串起 Lead、任务 DAG、成员执行、最终汇总与交付物，并在 Chat、Agents、Teams 三个页面提供统一视图。
head:
  - - meta
    - name: keywords
      content: Agent团队,任务板,看板,多Agent协作,派发,交付物,团队协作,AuraClaw
---
...
团队协作把这套项目机制搬进 AuraClaw：你建一个**团队**，指定一个 **Lead** 员工、若干**成员**员工；对 Lead 说一句目标，它把目标拆成任务落到**共享任务板**上；派发引擎把任务自动分给成员**并行执行**；成员完成后结果自动通报回 Lead，由它汇总、补派、直到整件事干完。你全程在 Teams 页旁观——或者直接往板上投任务。
