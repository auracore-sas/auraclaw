...
- **Workspace RBAC**: Owner / Admin / Member / Viewer roles + capability gates — AuraClaw is usable by a team for the first time
...
The employee turns **outward and finishes a whole job** — from a one-sentence brief to a publishable post — on AuraClaw's own primitives.
...
- **A2A interoperability**: AuraClaw acts as both A2A server and client with Agent Cards, streamed tasks, get/cancel, and constrained outbound calls
...
| **Native mobile app** | 8 IM channels + desktop (now with remote connect) + Web already cover it. On your phone, you use AuraClaw via DingTalk / Feishu / Telegram | When Web / IM channels can't deliver an irreplaceable mobile-only feature |
...
We're not building AuraClaw to chase ChatGPT, not to be the next Dify, not to add another buzzword to a funding deck.
