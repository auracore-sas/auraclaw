...
- **Ollama** — local GPU users; AuraClaw auto-detects `localhost:11434`
...
- **Backend never boots** — Check the log file (macOS: `~/Library/Application Support/AuraClaw/logs/mateclaw.log`; Windows: `%APPDATA%\AuraClaw\logs\mateclaw.log`). The desktop app picks a dynamic port — any port conflict is reported clearly in the log.
