...
|------|--------------------|------------------|
| **`builtin`** | Ships with AuraClaw under `skills/` in the classpath | The core team |
...
- **Built-in** — skills that ship with AuraClaw
...
A skill that's installed isn't necessarily a skill that runs — it might need an API key, a CLI tool, a AuraClaw feature flag toggled on.
...
ACP (Agent Client Protocol) is a protocol that lets external agent clients (Claude Code, Codex, other compatible clients) plug into AuraClaw as skills.
