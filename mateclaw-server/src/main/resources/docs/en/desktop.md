...
That's the desktop app in four sentences. No Java to install. No browser to open. No docker compose file. No port to remember. AuraClaw's desktop edition bundles Electron, a JRE 21 runtime, and the packaged Spring Boot server JAR into a single installer. **Your users never know Java is underneath.**
...
The WeCom QR-code authorization flow **must open in an in-app popup** (not the system browser) so the `postMessage` callback works. AuraClaw handles this in `setWindowOpenHandler` — `work.weixin.qq.com` domain opens as an in-app popup window.
