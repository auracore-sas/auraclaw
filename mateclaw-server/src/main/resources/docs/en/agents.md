...
description: AuraClaw's multi-agent system runs in two modes — ReAct for real-time reasoning and Plan-and-Execute for complex task decomposition. Agents can delegate to one another for true multi-agent collaboration.
head:
  - - meta
    - name: keywords
...
Not every question deserves deep reasoning, but some do. AuraClaw lets you turn on deep thinking per agent, per conversation:
