...
description: Install DeepSeek Harness and configure it as a digital employee runtime in AuraClaw.
head:
  - - meta
    - name: keywords
...
This guide connects the official DeepSeek Harness (DSH) to AuraClaw and creates a digital employee powered by the DSH runtime.

In AuraClaw, DSH is an **employee runtime**, not an MCP tool and not a regular plugin. MCP supplies tools; DSH owns the external Agent process, the ReAct loop, and the event stream that AuraClaw projects into the conversation UI.
...
AuraClaw remains responsible for employees, sessions, permissions, workspaces, message persistence, and UI projection. DSH runs the turn. AuraClaw injects the API key from the DeepSeek provider configuration into the DSH child process. Never put secrets in `runtimeConfig`, employee prompts, or the repository.

## Prerequisites

- macOS, Linux, or Windows (commands below use macOS / Linux syntax)
- JDK 21
- A running AuraClaw backend and frontend
- A DeepSeek API key
...
Keep the DSH binary outside the AuraClaw source tree. Configure its location with environment variables.

## Configure the IDEA backend

Open **Run | Edit Configurations...** in IDEA, select the AuraClaw Spring Boot configuration, and add these variables under **Environment variables**:
...
The default DSH model is `deepseek-v4-flash`. If the employee has no explicit model, AuraClaw uses the global model name and injects credentials from the `deepseek` provider. A custom model must be usable by the DeepSeek provider route in DSH.
...
`DSH_CORDIS_CONFIG` must point to the actual `cordis.yml`, not only the package directory. If a package directory is provided, AuraClaw also checks its `runtime/cordis.yml` child path.

### The answer appears twice

Use the latest backend version. DSH emits text deltas followed by a final assistant snapshot. AuraClaw must project the deltas only and must not append the snapshot again.
...
| Plugin | Extending AuraClaw tools, models, channels, or memory | No |
| DSH employee runtime | Hosting the DeepSeek Harness Agent loop and process | It is the employee runtime, not a tool |

The recommended composition is: **DSH as the employee runtime, MCP as the tool layer, and AuraClaw as the governance and visualization layer**.

## Security recommendations

- Store API keys only in AuraClaw provider configuration or a controlled environment.
