...
The admin console is the Vue 3 SPA that ships with every AuraClaw deployment. It runs in your browser (or inside the Electron desktop window), talks to the Spring Boot backend over REST + SSE, and exposes every capability AuraClaw has — chat, agents, knowledge, tools, skills, channels, security, cron jobs, usage analytics — behind one login.
...
AuraClaw uses domain-driven Pinia stores. Each store owns its slice of state exclusively.
