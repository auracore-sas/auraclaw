...
**Dos rutas de render (lección):** el texto de una respuesta se pinta por la **vista segmentada** (`ContentSegment`, una por segmento) y por la **vista clásica** (`MessageBubble.renderedContent`). El primer fix de citas solo cubrió la segunda, y el usuario veía la primera (ancla sin `href` → cursor igual, pero clic capturado por el delegador wiki). Ahora la prop `wikiCitations` viaja por ambas y `.wiki-citation` tiene estilo propio (punteado + pointer). **Toda opción nueva del renderizador debe pasar por las dos rutas.**

**Fix de las «Fuentes» que parecían apuntar a localhost (2026-09-21):** el pre-procesador de citas wiki actuaba **por forma** sobre cualquier bloque `Fuentes:` y convertía fuentes **web** en citas wiki con `href="#"` (de ahí que el navegador mostrara la URL del chat al pasar el ratón y que el clic buscara una página wiki inexistente). Ahora la decisión llega del contexto (¿el turno usó tools `wiki_*`?) vía la opción `wikiCitations`, los anclajes no llevan `href`, y las URLs reales de las fuentes quedan autoenlazadas. **Pendiente opcional**: construir la tabla de fuentes desde los resultados reales de la búsqueda web (llevan URL) para que «Fuentes» sea clicable incluso cuando el modelo solo escribe nombres.

**Endurecimiento + texto para el modelo (2026-09-21):** CSP `sandbox` para `image/svg+xml` en el endpoint de archivos, y el texto de los tools ahora explica que **la plataforma sirve y renderiza** los archivos (prohibido comentar el host: el agente del caso real especuló sobre `localhost` y un endpoint roto). Además las imágenes se entregan **ya en formato `![name](url)`**: `render_html_image`, `capture_screenshot` y `WorkspaceArtifactSurfacer` (gráficas de `execute_code`), así el resultado no depende de que el modelo acierte el formato.

**Fix de imágenes que no se veían (2026-09-21):** el asistente escribió `![grafica.png](http://localhost:18080/api/v1/files/generated/<id>)` y no se mostraba nada: los tokens de imagen de markdown no pasan por `link()` (sin override de `image()` → `<img>` sin `data-generated-image` → sin auth → 401) y el host que el modelo repite lo resuelve el navegador contra sí mismo (desde otra máquina, nunca carga). Añadido el override de `image()`, `data-generated-src`, y el helper `sameOriginFilePath()` aplicado en renderizador, panel, diálogo de previsualización y loader. Test de regresión con el texto verbatim del caso reportado.

**Imágenes visibles en la UI (2026-09-21):** las gráficas y elementos avanzados solo se podían **descargar** (`previewKindOf()` no tenía especie `image`). Se añadió el visor `ImagePreview.vue` (zoom + paneo + abrir en pestaña solo raster; SVG siempre dentro de `<img>` por seguridad), el clic en imágenes inline del mensaje ahora abre el visor, y el panel de archivos muestra **miniaturas** perezosas (IntersectionObserver + loader autenticado global). El test del upstream que fijaba `null` para imágenes quedó adaptado a `'image'` con comentario (⚠️ conservar en merges).

**Sin duplicación (2026-09-21):** al abrir una conversación antigua se veían los archivos **en dos paneles** («Resumen de Ejecución» listaba el último turno; el nuestro, toda la conversación). Se eliminó la sección de archivos del panel del upstream (`RunOverviewPanel`), que ahora solo muestra plan + subagentes; los archivos viven en un único panel. Ejemplo verificado: en `conv_1787335624586_kxf10p` antes salían 2 filas en el rail y 4 en el nuestro (2 duplicadas).
...
- **Resuelto (2026-09-21):** (a) + (c) implementados — el panel **aprende del 404/410** (`composables/useUnavailableFiles.ts`, alimentado por el handler global de descargas y por el diálogo de previsualización) y marca esas filas como no disponibles (tachadas, sin enlace, contador en el riel) con un botón «Ocultar no disponibles (N)» persistido. Queda sin hacer la opción (b) (exponer TTL/existencia desde el servidor), innecesaria mientras el TTL sea largo.
