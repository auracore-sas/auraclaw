export default {
  ...
  settings: {
    ...
    model: {
      ...
      usage: {
        ...
        chat: '聊天',
        wiki: '知识库',
        hint: '聊天：可用于对话（默认）。取消勾选“聊天”可将模型专用于内部任务（如知识库消化）——普通聊天不会使用它，但知识库仍可选择。',
        updated: '用途已更新',
        updateFailed: '用途更新失败',
      },
      ...
      claudeCodeOauthRevokeHint: '请在 Claude Code 客户端中退出登录。AuraClaw 不会修改 Claude Code 的本地凭据。',
    ...
    localTools: {
      desktopOnly: '本地工具仅在桌面客户端内可用。请在 AuraClaw 桌面应用中打开此页面进行管理。',
      enableLabel: '启用本地工具',
  ...
  agents: {
    ...
    fields: {
      ...
      runtimeNativeHint: '使用 AuraClaw 原生 Agent Loop。',
  ...
  acp: {
    ...
    env: {
      ...
      hints: {
        ...
        dashscope: '阿里云 DashScope key — 跟 AuraClaw .env 里的 DASHSCOPE_API_KEY 同一个值',
  ...
  login: {
    ...
    heroDesc: 'AuraClaw 的重点不是把更多能力堆进聊天框，而是把上下文、知识、执行和记忆收拢成一个可靠的工作面。',
  ...
  channels: {
    ...
    webchatHint: 'Web / API 接入既可把 AuraClaw 聊天挂件嵌入外部网站，也可作为纯后端 SSE 接口直接调用。配置 API Key 后，前端可引入 WebChat SDK；后端可直接 POST /api/v1/channels/webchat/stream，头带 X-MC-Key、体传 {message, visitorId, 可选 sessionId/agentId}。',
